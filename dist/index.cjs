...
const fs = require('fs');
const loroCrdtV015 = require('loro-crdt-v015');
const loroCrdtV016 = require('loro-crdt-v016');
const loroCrdt = require('loro-crdt');
...
const migrate = (inputSnapshot, from) => {
  ...
  switch (from) {
    case 0 /* V015 */:
      const loro15 = new loroCrdtV015.Loro();
      loro15.import(inputSnapshot);
      jsonOld = loro15.exportJsonUpdates();
      break;
    case 1 /* V016 */:
      const loro16 = new loroCrdtV016.Loro();
...
const args = yargs__default(helpers.hideBin(process.argv)).usage("Usage: loro-migrate [OPTIONS...]").option("input", {
  alias: "i",
...
const data = fs__default.readFileSync(inputPath);
const snapshot = migrate(data, from);
fs__default.writeFileSync(outputPath, snapshot);
console.log("\u{1F99C} Snapshot converted successfully");

exports.LoroVersion = LoroVersion;
